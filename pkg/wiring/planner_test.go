...
// only thing that costs real money per request. If it ever falls out of the
// proxy's rate limit, a public keyless endpoint becomes an open bill.
func TestPlannerIsRateLimited(t *testing.T) {
	...
	if !strings.Contains(limiter, `pathname === "/plan"`) {
		t.Errorf("the rate limit branch does not cover /plan: %s", limiter)
	}
}

// The route lives outside /api on purpose: Vercel routes /api/* to the Go
// functions, and a Next handler there would be ambiguous at best.
func TestPlannerRouteIsWired(t *testing.T) {
	root := repoRoot(t)

	route := read(t, root, "app", "plan", "route.ts")
	if !strings.Contains(route, "export async function POST") {
		t.Error("app/plan/route.ts does not export POST")
	}
	// Without a key the route must refuse cleanly rather than throw, so the
	// prompt can fall back to a command with a sentence explaining why.
	if !strings.Contains(route, "OPENAI_API_KEY") || !strings.Contains(route, "503") {
		t.Error("app/plan/route.ts does not refuse cleanly when OPENAI_API_KEY is unset")
	}
	...
	if !strings.Contains(route, "store: false") {
		t.Error("app/plan/route.ts does not set store: false on the provider")
	}

	page := read(t, root, "app", "page.tsx")
	if !strings.Contains(page, `fetch("/plan"`) {
		t.Error("app/page.tsx never calls /plan, so the planner is unreachable from the prompt")
	}
